...
function hasGoalsDir(root) {
  return existsSync(join(resolve(root), "docs", "goals"));
}
...
function skillRootPath() {
  ...
  return resolve(process.env.GOALBUDDY_SKILL_ROOT || join(cursorHome, "skills", "goalbuddy"));
}
...
function isForcedWorkspaceOverride() {
  const value = String(process.env.GOALBUDDY_WORKSPACE_FORCE || "").trim().toLowerCase();
...
export function registerKnownWorkspace(workspaceRoot) {
  ...
  if (!hasGoalsDir(root)) {
    return { ok: false, path: root, reason: "missing docs/goals/" };
  }
...
export function collectWorkspaceCandidates() {
  ...
  if (process.env.GOALBUDDY_WORKSPACE) {
    addCandidate(seen, candidates, process.env.GOALBUDDY_WORKSPACE);
  }
...
export function parseGoalRef(goalRef) {
  ...
  const absoluteMatch = ref.match(/^([A-Za-z]:.*?)\/docs\/goals\/([^/]+)(?:\/state\.yaml)?\/?$/);
  if (absoluteMatch) {
    return {
  ...
  const relativeMatch = ref.match(/(?:^|\/)docs\/goals\/([^/]+)(?:\/state\.yaml)?\/?$/);
  if (relativeMatch) {
...
export function resolveWorkspaceForGoal(goalRef, options = {}) {
  ...
  if (parsed.workspaceRoot) {
    const statePath = join(parsed.workspaceRoot, "docs", "goals", parsed.slug, "state.yaml");
  ...
  for (const root of collectWorkspaceCandidates()) {
    const statePath = join(root, "docs", "goals", parsed.slug, "state.yaml");
    if (existsSync(statePath)) {
...
export function getWorkspaceRoot() {
  const explicit = process.env.GOALBUDDY_WORKSPACE
    ? resolve(process.env.GOALBUDDY_WORKSPACE)
    : null;
  ...
  if (explicit && hasGoalsDir(explicit)) {
    return explicit;
  }
...
export function resolveGoalStatePath(goalRef, workspaceRoot) {
  const root = resolve(
    workspaceRoot !== undefined && workspaceRoot !== null
      ? workspaceRoot
      : resolveWorkspaceForGoal(goalRef),
  );
  const ref = String(goalRef || "").trim().replace(/\\/g, "/");
  if (!ref) {
    throw new Error("goal slug or path is required.");
  ...
  let candidate;
  if (parsed.slug && (!ref.includes("/") || ref.includes("docs/goals/") || ref.endsWith("state.yaml"))) {
    candidate = join(root, "docs", "goals", parsed.slug, "state.yaml");
...
export function resolveGoalDir(goalRef, workspaceRoot) {
